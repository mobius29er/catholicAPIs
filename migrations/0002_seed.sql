...
INSERT INTO apis (slug, name, tagline, description, homepage_url, docs_url, repo_url, kind, track, launched_at, platforms, pricing, pricing_note, open_source, license, auth, cors, official, categories, languages, status)
VALUES ('magisterium-ai-api', 'Magisterium AI API', 'Answers grounded in Church documents, with citations, over an OpenAI-compatible API.', 'A retrieval-augmented answer engine trained on the corpus of Church teaching — councils, encyclicals, the Catechism, canon law — that returns citations with every answer rather than free-associating. The API is OpenAI-compatible, so most existing SDKs and agent frameworks work against it by changing the base URL. Billing is pay-as-you-go on input and output tokens; the console shows usage.', 'https://www.magisterium.com/developers', 'https://www.magisterium.com/developers', NULL, 'api', 'api', NULL, '[]', 'paid', 'Pay-as-you-go per token. See the developer pricing page for current rates.', 0, NULL, 'api-key', 'unknown', 0, '["AI & Search","Church Documents","Catechism & Doctrine"]', '["en","es","fr","it","pt","de","pl"]', 'published')
ON CONFLICT(slug) DO UPDATE SET
  name = excluded.name,
  tagline = excluded.tagline,
  description = excluded.description,
...
INSERT INTO apis (slug, name, tagline, description, homepage_url, docs_url, repo_url, kind, track, launched_at, platforms, pricing, pricing_note, open_source, license, auth, cors, official, categories, languages, status)
VALUES ('sanctum-ipsum', 'sanctum-ipsum', 'Lorem ipsum, but from the saints.', 'A placeholder-text generator that fills your mockups with the words of the saints instead of Cicero. Usable from the website or as an API. Not serious infrastructure, and all the better for it.', 'https://github.com/graysonhicks/sanctum-ipsum', NULL, 'https://github.com/graysonhicks/sanctum-ipsum', 'api', 'api', NULL, '[]', 'free', NULL, 1, NULL, 'none', 'unknown', 0, '["Developer Tools","Quotes"]', '["en","la"]', 'published')
ON CONFLICT(slug) DO UPDATE SET
  name = excluded.name,
  tagline = excluded.tagline,
  description = excluded.description,
...
INSERT INTO apis (slug, name, tagline, description, homepage_url, docs_url, repo_url, kind, track, launched_at, platforms, pricing, pricing_note, open_source, license, auth, cors, official, categories, languages, status)
VALUES ('tamil-bible-database', 'Tamil Bible Database', 'Tamil scripture as a database dump with a PHP API.', 'The Tamil Bible in MySQL with a thin PHP API in front of it. Self-hosted rather than a service you call, which is exactly what you want if you are shipping to a region with patchy connectivity.', 'https://github.com/jayarathina/Tamil-Bible-Database', NULL, 'https://github.com/jayarathina/Tamil-Bible-Database', 'dataset', 'api', NULL, '[]', 'free', NULL, 1, NULL, 'none', 'unknown', 0, '["Bible"]', '["ta"]', 'published')
ON CONFLICT(slug) DO UPDATE SET
  name = excluded.name,
  tagline = excluded.tagline,
  description = excluded.description,
...
INSERT INTO apis (slug, name, tagline, description, homepage_url, docs_url, repo_url, kind, track, launched_at, platforms, pricing, pricing_note, open_source, license, auth, cors, official, categories, languages, status)
VALUES ('romcal', 'romcal', 'Generate the Roman Rite calendar locally, no network call.', 'A JavaScript and TypeScript library that computes the liturgical calendar in-process: seasons, ranks, colours, national propers, for any year. Because it runs locally it works offline, inside a Worker, or at build time — a different trade-off from calling a calendar API, and usually the better one for a frontend.', 'https://github.com/romcal/romcal', 'https://github.com/romcal/romcal#readme', 'https://github.com/romcal/romcal', 'library', 'api', NULL, '[]', 'free', 'MIT-licensed npm package.', 1, NULL, 'none', 'unknown', 0, '["Liturgical Calendar"]', '["en","fr","es","it","pt","pl","cs","sk"]', 'published')
ON CONFLICT(slug) DO UPDATE SET
  name = excluded.name,
  tagline = excluded.tagline,
  description = excluded.description,
...
INSERT INTO apis (slug, name, tagline, description, homepage_url, docs_url, repo_url, kind, track, launched_at, platforms, pricing, pricing_note, open_source, license, auth, cors, official, categories, languages, status)
VALUES ('catechism-ccc-json', 'catechism-ccc-json', 'The Catechism of the Catholic Church as structured JSON.', 'All 2,865 paragraphs of the Catechism in JSON, keyed by paragraph number and preserving the part/section/chapter/article hierarchy. Ready to index for search or to embed for retrieval — the usual first ingredient in a Catholic RAG pipeline.', 'https://github.com/nossbigg/catechism-ccc-json', NULL, 'https://github.com/nossbigg/catechism-ccc-json', 'dataset', 'api', NULL, '[]', 'free', NULL, 1, NULL, 'none', 'unknown', 0, '["Catechism & Doctrine"]', '["en"]', 'published')
ON CONFLICT(slug) DO UPDATE SET
  name = excluded.name,
  tagline = excluded.tagline,
  description = excluded.description,
